...
class Serializator {
    ...
    public static function json_encode($object, $depth=1, $whitelist=array(), $blacklist=array()){
        return json_encode(Serializator::toArray($object, $depth, $whitelist, $blacklist));
    }
            ...
            if (is_object($aValue)){
                ...
                if (get_class($aValue) === 'DateTime'){
                    ...
                    foreach ($aValue as $val){
                        $collect[] = Serializator::toArray($val, $nextDepth, $whitelist, $blacklist);
                    }
                    $anArray[$prop->name] = $collect;

                    // Otherwise, we can simply make it an array
                }else{
                    $anArray[$prop->name] = Serializator::toArray($aValue, $nextDepth, $whitelist, $blacklist);
                }
